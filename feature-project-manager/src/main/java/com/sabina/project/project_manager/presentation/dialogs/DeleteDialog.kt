...
internal class DeleteDialog : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        return AlertDialog.Builder(requireContext(), R.style.AlertDialogTheme)
            .setTitle(getString(args.title))
            .setMessage(getString(R.string.project_overview_delete_dialog_subtitle).html())
            .setNegativeButton(R.string.cancel) { _, _ -> onClose(Result.NEGATIVE) }
            .setPositiveButton(R.string.delete) { _, _ -> onClose(Result.POSITIVE) }
            .show()
    }
